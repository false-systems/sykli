...
- Windows x86_64 release binaries (`sykli-vX.Y.Z-windows-x86_64.zip`). Graph
  runs, `verify`, `init`, `inspect` and `assess` run on Windows; tasks execute
  under a POSIX `sh` found on `PATH` (Git for Windows ships one). Typed
  production remains Linux and macOS.
