...
Read `AGENTS.md` first: it states the product boundaries (no server, daemon,
webhook, coordination or agent execution; no interpretation of results beyond
the declared predicate) and the removed capabilities that do not return without
a named user. Under a toimija session, the packet named in your launch prompt
is the workset contract; run `toimija verify` before committing.
