...
- **Phase 2** of the GitHub-native rollout: webhook → mesh placement → executor → check-run lifecycle. Per-task check runs transition through `queued` → `in_progress` → conclusion.
