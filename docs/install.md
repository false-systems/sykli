...
```bash
docker run --rm -v "$PWD:/repo" -w /repo ghcr.io/false-systems/sykli:v0.3.0-tools run sykli.json --json
docker run --rm ghcr.io/false-systems/sykli:v0.3.0 --help
```
...
```yaml
      - uses: actions/checkout@v7
        with:
          fetch-depth: 0
      - uses: false-systems/sykli@v0.3.0
        with:
          contract: sykli.json
```
